...
## Project knowledge — COMPLETE FILE INDEX

All authoritative source files live in the project knowledge base (the Claude.ai project
export). When running locally in Claude Code, unzip the project export into this folder so the
paths below resolve. This index covers **every** file in the knowledge base. File sizes are
approximate (from the project snapshot) and are included to flag the large files Claude Code
should read selectively rather than in full.

### Core reference / documentation (Markdown & Word)

| File | ~Size | Contents |
|------|-------|----------|
| `BE_FactoryGatewayEvents.md` | 1.2M | All BE (BE1 + BE2) named queries and script library functions. PRIMARY backend reference. Large — search/grep for specific query paths rather than reading whole. |
| `FE1_FactoryControl.md` | 1.3M | All FE (FactoryControl) named queries and script library functions. PRIMARY front-end reference. Large — search/grep rather than full read. |
| `BE_README.md` | 8K | Layout/orientation for the FactoryGatewayEvents BE1+BE2 compact bundle: which area each backend owns (BE1 = Frame/Boom/Paint/Run Schedule/JaMES/UserManagement; BE2 = Main Line/Subs/Legacy/Navithor/MQTT/webdev REST), shared vs be1_only/be2_only file split. Read first to navigate `BE_FactoryGatewayEvents.md`. |
| `JLG_Jefferson_City_Master_System_Reference.docx` | 60K | Full system architecture reference for the Jefferson City plant. |
| `CHANGELOG_PT030A_PaintUnload_GetNextWorkOrder_Refresh.md` | 8K | The getNextWorkOrder refresh pattern (getOrder + updateRunSchedules) — reference for `refreshNextWorkOrder`. |
| `Factory_Automation_PLC_Reference.md` | 60K | PLC tag structure. READ-ONLY reference; the SCC does NOT gate on PLC status. |
| `Sepasoft_Batch_Reference.md` | 48K | Sepasoft Batch configuration: unit class hierarchy, phases, recipes (consolidated from batchUnitClassExport / phaseExport / recipeExport). |
| `Run_Schedule_WIP_Status_Key.docx` | 4K | Key/legend for Run Schedule WIP status values. (Note: the project copy did not parse as readable text in this environment — confirm contents against the WIP status table above, which is the authoritative status map used by the SCC.) |

### Ignition tag exports (JSON)

| File | ~Size | Contents |
|------|-------|----------|
| `JLG_JeffersonCity_ignition_tags_TAGIO1_20260513_compact.json` | 940K | Compact tag export for the TAGIO1 provider (BE1 — Frame/Boom/Paint/Run Schedule). Includes UDT type defs and instances. Large — query for specific tag paths. |
| `JLG_JeffersonCity_ignition_tags_TAGIO2_20260513_compact.json` | 360K | Compact tag export for the TAGIO2 provider (BE2 — Main Line/Subs/Legacy/Navithor fleet incl. AMR Data). |

### Live DB table samples (CSV)

| File | Rows | Contents |
|------|------|----------|
| `WorkstationAssignment.csv` | 128 | Station roster: PCID, Area, StationName, JaMES/PartRequesting/Interlocks tab flags, printer config, batch recipe class/name, symbolic points, destination station. Source for the station-list query. |
| `MachineConfig.csv` | 1985 | Per-serial machine config key/value rows (SerialNumber, Description, Value). Source for `getMachineConfigInfo` / serial validation. Largest CSV. |
| `WIPWorkOrders_Assembly.csv` | 100 | Assembly WIP work orders: WG work order, serial, product code, station, timestamps, Status/AreaStatus/LineStatus. The Status=2 boundary check reads this table. |
| `WIPWorkOrders_PaintUnload.csv` | 65 | Paint Unload WIP work orders: WG work order, componentID, serial, weldment item, product code, station, timestamps, status, bypass lane, carrier number. |
| `Fabrication_FrameWIP.csv` | 50 | Frame fabrication WIP: componentID, NWG work order, item number, station, timestamps, status. |
| `fabrication_BoomWIP.csv` | 80 | Boom fabrication WIP: same shape as Frame WIP. |
| `PaintLoad_CarrierGroups.csv` | 74 | Paint load carrier groups: area/status, carrier group/order, kit number, WG serial, NWG componentID, weldment item, paint description, legacy flag, timestamps, carrier number. |
| `PaintEmptyCarriers.csv` | 42 | Empty paint carriers: station name, status, timestamps, carrier number. |
| `NonConformance.csv` | 335 | Non-conformance records: componentID, area, station, comment, report/update user, status, timestamps. |
| `WholeGoodToNonWholeGoodAssoc.csv` | 50 | WG↔NWG association: AssociationID, FinalSerialNumber, ComponentID, ItemName. |
| `ProductsTable.csv` | 31 | Product master: product code, machine type, item name, item number, name occurrence, model, legacy flag. |
| `ItemNumberConfig.csv` | 285 | Item-number config key/value rows: ItemNumber, Description, Value, ValueDataType. |
| `GatewayConfig.csv` | 11 | Gateway/environment config: Environment, ConfigName, ConfigType. Backs `GatewayConfig.getBackEnd_1/2()`. |

### Worksheets & standards (Excel)

| File | ~Size | Contents |
|------|-------|----------|
| `Run_Schedule_Station_Control_Config_Worksheet.xlsx` | 100K | Run Schedule / Station Control configuration worksheet. |
| `JaMES__Station_Control_Mapping_Worksheet_2025_03_24.xlsx` | 28K | JaMES ↔ Station Control mapping worksheet. |
| `Programming_Standards.xlsx` | 24K | MDI/JLG programming standards. |
| `Test_Case_Document_Standard__rev_250326.xlsm` | 52K | Test case document standard/template (macro-enabled) — the JLG/MDI test case workbook format. |

### Flow / process diagrams (PDF)

| File | ~Size | Contents |
|------|-------|----------|
| `AMR_to_Manual_Station_Flow_9325.pdf` | 96K | AMR → manual station process flow. |
| `Station_Work_Flow_12_4_24.pdf` | 260K | Station work flow diagram. |
| `Machine_Status__Flowchart_31025.pdf` | 252K | Machine status flowchart. |
...
| Active WIP boundary check | FactoryControl | `station-control-center/getActiveStations` (NEW; param `StationName`; `activeWipCount` fail-safe → 0 if absent) |
...
| Schedule NWG for pre-paint (INSERT Status 1 at TW0700/TT170A/TT170B) | FactoryGatewayEvents (BE1) | `RunSchedule/Fabrication/Select/insertToWIP` (params `component`,`item`,`stn`,`stn5`,`stnType`; `{stnType}` picks Frame/BoomWIP; the query behind the BE `runQueryHelper` "InsertToWIPWorkOrders" label — called directly, no `buildCarrierGroup`) |
| Pre-assembly TF000X | FactoryGatewayEvents | `RunSchedule/PaintUnload/insert/insertFabricationToPreAssemblyInventory` |
| Component schedule (work-order recon, NWG) | FactoryControl | `station-control-center/getFabricationScheduleByComponent` (NEW; param `componentID`) |
| Serial schedule (work-order recon, WG) | FactoryControl | `station-control-center/getAssemblyScheduleBySerial` (NEW; param `serialNumber`) |
| Mark WG complete (work-order recon) | FactoryGatewayEvents | `RunSchedule/Assembly/Update/WorkOrderComplete` |
...
## New named queries (3 total — was originally one)

The work-order reconciliation feature added two more small, read-only, single-key
FactoryControl queries (`station-control-center/getFabricationScheduleByComponent` param
`componentID`, and `station-control-center/getAssemblyScheduleBySerial` param `serialNumber`).
All three must be deployed. The original boundary-check query:

**Path:** `station-control-center/getActiveStations`  (CONFIRMED deployed; param casing matters)
**Project:** FactoryControl  **Type:** Scalar  **DB:** SCADA
**Param:** `StationName` (String, capital S)
...
3. One new named query at `station-control-center/getActiveStations` (param `StationName`) — confirmed already deployed in the lab gwbk
